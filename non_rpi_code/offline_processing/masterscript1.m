% Eddy Current - Golden Template Master Alignment & Lift-off Analysis
clear; clc; close all;
...
test_folders = {'measurements 30mm', 'measurements 50mm'};
...
%% Phase 1: Extract the "Golden Template" (30mm / run1)
...
for f = 1:length(test_folders)
    ...
    for r = 1:num_runs
        ...
        for h = 0:(num_harmonics-1)
            ...
            % For 50mm, noise is high. Lower prominence threshold to find the weak signal
            min_height = max_env * 0.10; 
            [~, locs] = findpeaks(mag_envelope, 'MinPeakDistance', min_distance, 'MinPeakHeight', min_height);
            ...
            if length(locs) >= num_bars
                for b = 1:num_bars
                    ...
                    peak_areas(r, h+1, b) = trapz(final_snippet);
    ...
    % Wraps phase diff to standard -180 to 180 circle
    avg_phase_diff_wrapped = mod(avg_phase_diff + 180, 360) - 180;
