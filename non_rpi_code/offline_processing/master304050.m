% Eddy Current - Consolidated Master Alignment & Lift-off Analysis (30mm, 40mm, 50mm)
clear; clc; close all;
...
template_dir = fullfile('measurements 30mm', 'run1');
...
for h = 0:(num_harmonics-1)
    ...
    [~, locs] = findpeaks(mag_envelope, 'MinPeakDistance', 200, 'MinPeakHeight', min_height);
    
    if length(locs) >= num_bars
        locs = locs((end-num_bars+1):end); % Ensure we only take the last 3
        for b = 1:num_bars
            rough_center = locs(b);
...
for f = 1:length(test_folders)
    ...
    for r = 1:num_runs
        ...
        for h = 0:(num_harmonics-1)
            ...
            % 5% catches the weak signals in higher lift-off folders (40mm, 50mm)
            min_height = max_env * 0.05; 
            [~, locs] = findpeaks(mag_envelope, 'MinPeakDistance', min_distance, 'MinPeakHeight', min_height);
                
            if length(locs) >= num_bars
                % Ignore startup noise, take the last configured number of bars
                locs = locs((end-num_bars+1):end);
                
                for b = 1:num_bars
                    rough_center = locs(b);
                    
                    % Extract rough noisy target
                    ...
                    % Extract Phase/Mag at the new perfectly aligned peak
    ...
    for h = 1:num_harmonics
        for b = 1:num_bars
            ...
            % Safe reshape to prevent 1-run crashing
            ...
            % Styling configuration with text sizes increased by an additional ~20%
            title(sprintf('Harmonic %d: %s', h-1, bar_names{b}), 'FontSize', 17, 'FontWeight', 'bold');
            xlabel('Packet #', 'FontSize', 16, 'FontWeight', 'bold');
            ylabel('Magnitude', 'FontSize', 16, 'FontWeight', 'bold');
            set(gca, 'FontSize', 14);
            
            grid on; xlim([-window_half_width, window_half_width]);
            if subplot_idx == 1, legend('Location', 'northeast', 'FontSize', 12); end
        end
    end
    ...
    % Use circular statistics to correctly wrap variance and averages
