% Eddy Current - Speed Variance Analysis
clear; clc; close all;
...
smooth_window = 15;           
baseline_window = 500; 
integration_half_width = 80; % Reduced to fit narrow fast sweeps

% --- Sweep Detection Parameters ---
tracker_window = 100; 
min_sweep_distance = 100; % Reduced to catch sweeps spaced closely together
...
min_sweep_height = max(sweep_tracker) * 0.10; 
...
%% Summary Output
mag_variance_pct = (std(sweep_mags) / mean(sweep_mags)) * 100;
area_variance_pct = (std(sweep_areas) / mean(sweep_areas)) * 100;
