...
class ViewProcessed(ViewCommon):

    def __init__(self, tab_widget=None):
        this_tab = QWidget()
        tab_layout = QHBoxLayout()
        this_tab.setLayout(tab_layout)
        tab_widget.addTab(this_tab, "Processing")
        ...
        self.line_plots = [None] * 2
        for k in range(2):
            self.line_plots[k] = self._init_line_plot(self._plot_widgets[k], 'Sample', 'Re', 'r', 2)
            self.set_plot_range(self._plot_widgets[k], (0, self.n_samples), (0, 50), pad=(0, 0))
        ...
        self.line_data = [[0.0] * self.n_samples] * 2
    ...
    def update(self, md_data):
        ...
        self.line_plots[0].setData(x=self.samples, y=self.line_data[0])
        self.line_plots[1].setData(x=self.samples, y=self.line_data[1])

        self.update_fps()

    def update_scrolling_plot_axis(self):
        if self.use_reim:
            self._plot_widgets[0].setLabel('left', 'Re')
            self._plot_widgets[1].setLabel('left', 'Im')
        else:
            self._plot_widgets[0].setLabel('left', 'Mag')
    ...
    def set_components(self):
        if self.component_ctrl[0].isChecked():
            ...
            self.use_reim = False
        self.update_scrolling_plot_axis()
